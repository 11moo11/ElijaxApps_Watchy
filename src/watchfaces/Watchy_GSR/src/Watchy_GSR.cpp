...
void WatchyGSR::init(String datetime){
    ...
    if (B || Updates.Full || WatchTime.NewMinute || WfNM){
        ...
        // BMA initialization is now handled by core Watchy firmware

#ifdef STABLEBMA_H_INCLUDED
        // Removed redundant sensor reinitialization - use core Watchy._ensureBmaReady() instead
#endif
...
void WatchyGSR::showWatchFace(){
  ...
  UiSDK::displayUpdate(display, !Updates.Full); //partial refresh
...
void WatchyGSR::drawWatchFace(){
    UiSDK::fillScreen(display, BackColor());
    UiSDK::setTextWrap(display, false);
    if (WatchStyles.AddOn[Options.WatchFaceStyle] == nullptr) { if (!OverrideBitmap()) { if (Design.Face.Bitmap) UiSDK::drawBitmap(display, 0, 0, Design.Face.Bitmap, 200, 200, ForeColor(), BackColor()); } } else { if (!WatchStyles.AddOn[Options.WatchFaceStyle]->OverrideBitmap()) { if (Design.Face.Bitmap) UiSDK::drawBitmap(display, 0, 0, Design.Face.Bitmap, 200, 200, ForeColor(), BackColor()); } }
    UiSDK::setTextColor(display, ForeColor());
...
uint16_t  lwidth, lheight;
    if (GSRLogOutput > ""){
      ...
      UiSDK::getTextBounds(display, GSRLogOutput, lleft, lbottom, &lx, &ly, &lwidth, &lheight);
      UiSDK::fillRect(display, lleft, 0, lwidth, lheight, BackColor());
      UiSDK::setCursor(display, lleft, lbottom + 16);
      UiSDK::print(display, GSRLogOutput);
      GSRLogOutput = "";
    }
}
...
void WatchyGSR::drawTime(uint8_t Flags){
    ...
    UiSDK::setFont(display, Design.Face.TimeFont);
...
void WatchyGSR::drawYear(){
    UiSDK::setFont(display, Design.Face.YearFont);
...
void WatchyGSR::drawMenu(){
    ...
    UiSDK::drawBitmap(display, 0, Design.Menu.Top, (Menu.Style == GSR_MENU_INOPTIONS) ? OptionsMenuBackground : MenuBackground, GSR_MenuWidth, GSR_MenuHeight, ForeColor(), BackColor());
    UiSDK::setTextColor(display, Options.LightMode && Menu.Style != GSR_MENU_INNORMAL ? GxEPD_WHITE : GxEPD_BLACK);
    switch (Menu.Item){
        case GSR_MENU_STEPS:
    ...
    UiSDK::getTextBounds(display, O, 0, Design.Menu.Header + Design.Menu.Top, &x1, &y1, &w, &h);
    w = (196 - w) /2;
    UiSDK::setCursor(display, w + 2, Design.Menu.Header + Design.Menu.Top);
    UiSDK::print(display, O);
    UiSDK::setTextColor(display, GxEPD_BLACK);  // Only show menu in Light mode
    ...
    if (O){
        ...
        UiSDK::getTextBounds(display, O, 0, D, &x1, &y1, &w, &h);
        /*
         * D = Down (vertical)
         * L = Left (horizontal)
         * B = Begin (horizontal)
        ...
        */
        L = ((196 - w) /2) + 2; B = L; C = 0;
        UiSDK::setCursor(display, L, Design.Menu.Data + Design.Menu.Top);
        if (Updates.Indexing.Active || Updates.Highlighting.Active){
            for (P = 0; P < O.length(); P++){
                ...
                if (Updates.Highlighting.Active) UiSDK::setTextColor(display, isBitOn(Updates.Highlighting.Bits,P) ? GxEPD_WHITE : GxEPD_BLACK);
                UiSDK::print(display, T);
                L = UiSDK::getCursorX(display);  /* Read the Cursor AFTER printing */
                if (Updates.Indexing.Active){
                    if (C > 0){
                        C--;
                        if (C == 0){
                            UiSDK::fillRect(display, B, D + 5, L - B, 3, GxEPD_BLACK);
                        }
                    }
                }
            }
        }else UiSDK::print(display, O);
    }
}
...
void WatchyGSR::drawData(String dData, byte Left, byte Bottom, WatchyGSR::DesOps Style, byte Gutter, bool isTime, bool PM){
    ...
    UiSDK::getTextBounds(display, dData, Left, Bottom, &X, &Y, &Width, &Height);
    ...
    UiSDK::setCursor(display, Left, Bottom);
    UiSDK::print(display, dData);

    if (isTime && PM){
        if (Style == WatchyGSR::dRIGHT) Left = constrain(Left - 12, Gutter, 200 - Gutter);
        else Left = constrain(Left + Width + 6, Gutter, 190);
        UiSDK::drawBitmap(display, Left, Bottom - Design.Face.TimeHeight, PMIndicator, 6, 6, ForeColor());
    }
}
...
void WatchyGSR::setFontFor(String O, const GFXfont *Normal, const GFXfont *Small, const GFXfont *Smaller, byte Gutter){
    ...
    UiSDK::setFont(display, Normal); UiSDK::getTextBounds(display, O, 0, 0, &x1, &y1, &w, &h);
    if (w > wi) { UiSDK::setFont(display, Small); UiSDK::getTextBounds(display, O, 0, 0, &x1, &y1, &w, &h); }
    if (w > wi) { UiSDK::setFont(display, Smaller); UiSDK::getTextBounds(display, O, 0, 0, &x1, &y1, &w, &h); }
 }
...
void WatchyGSR::setFontColor(uint16_t Color){
    ...
    UiSDK::setTextColor(display, Color);
}
...
void WatchyGSR::GoDark(bool DeepSleeping){
  ...
  if ((Updates.Drawn || !Darkness.Went) && (DeepSleeping || !Showing()))
  {
    ...
    UiSDK::fillScreen(display, GxEPD_BLACK);
    if (WatchStyles.AddOn[Options.WatchFaceStyle] == nullptr) { if (!OverrideSleepBitmap()) { if (Design.Face.SleepBitmap) UiSDK::drawBitmap(display, 0, 0, Design.Face.SleepBitmap, 200, 200, GxEPD_WHITE, GxEPD_BLACK); } } else { if (!WatchStyles.AddOn[Options.WatchFaceStyle]->OverrideSleepBitmap()) { if (Design.Face.SleepBitmap) UiSDK::drawBitmap(display, 0, 0, Design.Face.SleepBitmap, 200, 200, GxEPD_WHITE, GxEPD_BLACK); else B = OverrideSleepBitmap(); } }
    drawChargeMe(true);
    drawStatus(true);
...
void WatchyGSR::drawChargeMe(bool Dark){
  ...
  if (Battery.Direction == 1){
      UiSDK::drawBitmap(display, Design.Status.BATTx, Design.Status.BATTy, Charging, 40, 17, C);   // Show Battery charging bitmap.
  }else if (Battery.Read < getLowBattery(true) && Battery.Read > 3.2999f){ // At <3.3v, the display won't work (V3 workaround).
      UiSDK::drawBitmap(display, Design.Status.BATTx, Design.Status.BATTy, (Battery.Read < getLowBattery() ? ChargeMeBad : ChargeMe), 40, 17, C);   // Show Battery needs charging bitmap.
  }
}
...
void WatchyGSR::drawStatus(bool Dark){
  ...
  bool Ok = true;
  UiSDK::setFont(display, &Bronova_Regular13pt7b);
  UiSDK::setTextColor(display, C);
  if (WatchyStatus > "" && !Dark){
      Ok = false;
      if (WatchyStatus.startsWith(WiFiTXT)){
          UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iWiFi, 19, 19, C);
          if (WatchyStatus.length() > 4){
              UiSDK::setCursor(display, X + 17, Design.Status.WIFIy);
              UiSDK::print(display, WatchyStatus.substring(4));
          }
      }
      else if (WatchyStatus == "TZ")  UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iTZ, 19, 19, C);
      else if (WatchyStatus == "NTP") UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iSync, 19, 19, C);
      else if (WatchyStatus == "GL")  UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iGeo, 19, 19, C);
      else if (WatchyStatus == "WE")  UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iWeather, 19, 19, C);
      else if (WatchyStatus == "ESP") UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iSync, 19, 19, C);
      else if (WatchyStatus == "NC")  UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, iNoClock, 19, 19, C);
      else if (Alarms_Times[1] > 0 || Alarms_Times[2] > 0 || Alarms_Times[3] > 0 || Alarms_Times[4] > 0 || TimerDown.ToneLeft > 0) Ok = true;
      else{
          UiSDK::setCursor(display, X, Design.Status.WIFIy);
          UiSDK::print(display, WatchyStatus);
      }
  }

  if (Ok){
    if (TimerDown.ToneLeft > 0 && !Dark) { UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 15, Tone_C, 12, 14, C); X += 13; Ok = false; }
    if (Alarms_Times[0] > 0 && !Dark) { UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 15, Tone_1, 12, 14, C); X += 13; Ok = false; }
    if (Alarms_Times[1] > 0 && !Dark) { UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 15, Tone_2, 12, 14, C); X += 13; Ok = false; }
    if (Alarms_Times[2] > 0 && !Dark) { UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 15, Tone_3, 12, 14, C); X += 13; Ok = false; }
    if (Alarms_Times[3] > 0 && !Dark) { UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 15, Tone_4, 12, 14, C); Ok = false; }
    if (Ok && Options.GameStatus) { UiSDK::drawBitmap(display, X, Design.Status.WIFIy - 18, GameIcon, 19, 18, C); Ok = false; }
...
void WatchyGSR::DisplayInit(bool ForceDark){
  ...
  if (Updates.Init){
    ...
    UiSDK::init(display, 0,Rebooted,10,true);  // Force it here so it fixes the border.
    Updates.Init=false;
    Rebooted=false;
  }
}
...
void WatchyGSR::DisplaySleep(){ if (!Updates.Init) { Updates.Init = true; UiSDK::hibernate(display); } }
